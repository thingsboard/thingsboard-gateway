...
class Pointer:
    def __init__(self, path_to_folder, log, settings, state_file_name="state.txt"):
        self.__path_to_folder = path_to_folder
        self.__log = log
        self.__settings = settings
        self.__state_file_lock = Lock()
        self.__directory = os.path.dirname(path_to_folder)
        # TODO: What if file doesn't provided in settings? Let's use search in folder instead
        self.__read_database_file = self.__settings.data_folder_path.split("/")[-1]
        self.__write_database_file = self.__settings.data_folder_path.split("/")[-1]
        self.__read_position = 0
        self.__state_file_name = state_file_name
        self.__full_path_to_state_file = os.path.join(
            self.__directory, self.__state_file_name
        )
        if os.path.exists(self.__full_path_to_state_file):
    ...
    def sort_db_files(self):
        all_files = os.listdir(self.__directory)
    ...
    def __write_info_to_state_file(self):
        try:
            state_node = {
                ...
                "read_position": self.__read_position,
    ...
    def read_and_apply_state_file(self):
        ...
        with self.__state_file_lock:
            try:
                ...
                if state_data_node:
                    self.__read_position = state_data_node["read_position"]
