...
class ModbusConnector(Connector, threading.Thread):
    ...
    def __process_devices(self):
        for device in self.__devices:
            ...
            device_responses_always = {"timeseries": {},
                                "attributes": {},
                                }
            to_send = {}
            try:
                for config_data in device_responses:
                    if self.__devices[device]["config"].get(config_data) is not None:
                        ...
                        if self.__devices[device]["next_"+config_data+"_check"] < current_time:
                            ...
                            for interested_data in range(len(self.__devices[device]["config"][config_data])):
                                ...
                                #     continue
                                if not current_data["sendDataOnlyOnChange"]:
                                    #sendataonlyonchange FALSE for variable
                                    device_responses_always[config_data][current_data["tag"]] = {"data_sent": current_data,
                                                                                      "input_data": input_data}
                                elif current_data["sendDataOnlyOnChange"]:
                                    #sendataonlyonchange TRUE for variable
                                    device_responses[config_data][current_data["tag"]] = {"data_sent": current_data,
                                                                                      "input_data": input_data}
                                elif self.__devices[device]["config"].get("sendDataOnlyOnChange"):
                                    #sendataonlyonchange TRUE for variable
                                    device_responses[config_data][current_data["tag"]] = {"data_sent": current_data,
                                                                                      "input_data": input_data}
                                elif not self.__devices[device]["config"].get("sendDataOnlyOnChange") or self.__devices[device]["config"].get("sendDataOnlyOnChange") is None:
                                    #sendataonlyonchange FALSE for variable
                                    device_responses_always[config_data][current_data["tag"]] = {"data_sent": current_data,
                                                                                      "input_data": input_data}
                            ...
                            log.debug(device_responses_always)
                        converted_data = {}
                        try:
                            ...
                            converted_data_always = self.__devices[device]["converter"].convert(config={**self.__devices[device]["config"],
                                                                                                 "byteOrder": self.__devices[device]["config"].get("byteOrder", self.__byte_order),
                                                                                                 "wordOrder": self.__devices[device]["config"].get("wordOrder", self.__word_order)},
                                                                                         data=device_responses_always)
                        except Exception as e:
                            log.error(e)

                        #to_send only on change (converted_data)
                        if converted_data:
                            ...
                            if not to_send.get("attributes") and not to_send.get("telemetry"):
                             ...
                             #Here we do store last_telemetry and last_attribute because it is compulsory to check in future reads

                        
                        #to_send always (converted_data_always)
                        if converted_data_always:
                            self.statistics['MessagesReceived'] += 1
                            to_send_always = {"deviceName": converted_data_always["deviceName"],
                                       "deviceType": converted_data_always["deviceType"]}
                            to_send_always["telemetry"] = converted_data["telemetry"]
                            to_send_always["attributes"] = converted_data_always["attributes"]
                            #Here we do not store last_telemetry or last_attribute because it is not necessary to check in future reads
                    ...
                    if to_send_always.get("attributes") or to_send_always.get("telemetry"):
                        self.__gateway.send_to_storage(self.get_name(), to_send_always)
                        self.statistics['MessagesSent'] += 1
